...
```{r, warning=FALSE, message = FALSE}
ggplot(dfNew1, aes(x = BODY_WEI)) +
        geom_histogram(aes(y = ..density..)) +
        labs(x = "", y = "") +
        geom_line(aes(y = dnorm(BODY_WEI, mean = mean(BODY_WEI, na.rm=TRUE), 
                                sd = sd(BODY_WEI, na.rm=TRUE))), color = "blue")
ggplot(dfNew1, aes(x = SLEEP)) +
        geom_histogram(aes(y = ..density..)) +
        labs(x = "", y = "") +
        geom_line(aes(y = dnorm(SLEEP, mean = mean(SLEEP, na.rm=TRUE), 
                                sd = sd(SLEEP, na.rm=TRUE))), color = "blue")

ggplot(dfNew5, aes(x = BODY_WEI)) +
        geom_histogram(aes(y = ..density..)) +
        labs(x = "", y = "") +
        geom_line(aes(y = dnorm(BODY_WEI, mean = mean(BODY_WEI, na.rm=TRUE), 
                                sd = sd(BODY_WEI, na.rm=TRUE))), color = "blue")
ggplot(dfNew5, aes(x = SLEEP)) +
        geom_histogram(aes(y = ..density..)) +
        labs(x = "", y = "") +
        geom_line(aes(y = dnorm(SLEEP, mean = mean(SLEEP, na.rm=TRUE), 
                                sd = sd(SLEEP, na.rm=TRUE))), color = "blue")
...
ks.test(dfNew2$BODY_WEI, "pnorm", mean = mean(dfNew2$BODY_WEI, na.rm=TRUE), sd = sd(dfNew2$BODY_WEI, na.rm=TRUE))
ks.test(dfNew2$SLEEP, "pnorm", mean = mean(dfNew2$SLEEP, na.rm=TRUE), sd = sd(dfNew2$SLEEP, na.rm=TRUE))
```
Тесты не отвергли гипотезу о нормальности признаков внутри групп.

Итак. Группы у нас независимые и  имеют нормальное распределение, поэтому будем 
использовать критерии для независимых с.в., а t-тесты будут точными, если все будет
хорошо с дисперсиями.
...
t.test((dfNew1$SLEEP - mean(dfNew1$SLEEP, na.rm = TRUE)) * 
       ...
       (dfNew5$SLEEP - mean(dfNew5$SLEEP, na.rm = TRUE)) * 
         (dfNew5$SLEEP - mean(dfNew5$SLEEP, na.rm = TRUE)))
```
Видно, что тест Левена менее мощный, так как он асимптотический из-за отсутствия 
нормальности у квадратов. Для веса дисперсии одинаковые, а для сна нет. 
...
t.test(dfNew1$SLEEP, dfNew5$SLEEP, alternative="greate")
```
Потчти все гипотезы о равенстве мат. ожиданий отверглись в пользу того, что
животное, которое спит в более защищенном месте весит меньше и спит больше, 
