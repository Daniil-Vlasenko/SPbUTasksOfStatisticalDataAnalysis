...
состоит из целых чисел, поэтому будем ситать ее дискретным признаком. Остальные
...
нормальность. Проверим это графически и тестами.
```{r, warning=FALSE, message = FALSE}
...
Посмотрим на t-тесты с равными и неравными дисперсиями для независимых выборок.
```{r}
...
что мы наблюдали на box-plot-ах выше. Все отверглось как и предполагалось.

5, 6. Так как в случае сна удалось проверить гипотезу только асимптотическим критерием,
а выборкм у нас небольшие, воспользуемся непараметрическим t-критерием. А именно
критерием Вилксона.
```{r}
...
Наконец используем двухвыборочный тест Колмогорова-Смирнова, который имеет 
...
жизни и периода вынашивания потомства, 2) положительную корреляцию корреляцию фаз 
...
3. Теперь посмотрим на ранговоые корреляции между прихнаками.
```{r, warning=FALSE, message = FALSE}
dplyr::select(dfNew, -NAME) %>%
mutate(PRED_IND = as.numeric(PRED_IND), EXP_IND = as.numeric(EXP_IND), DANG_IND = as.numeric(DANG_IND)) %>%
...
Итог. 1) Животные с большим весом спят меньше и в более опасных местах. Второе логично, 
потому что они не могут залеть в дупло, первое контринтуитивно, но гугл подверждает 
этот факт, причина его пока не известна. 2) Животные, находящиеся в большей опасности 
